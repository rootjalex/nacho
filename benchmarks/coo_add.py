"""COO addition: the generated nacho kernel against PyTorch sparse.
...
shape, and plots runtime against the combined nnz of the two operands.
    ...
    python benchmarks/coo_add.py --device cpu --start 0 --end 1600
"""
...
def benchmark_coo_add(start, end, device="cpu", save_and_plot=True):
    """Nacho vs PyTorch COO add over consecutive matrix pairs."""
    on_gpu = device == "cuda"
    ...
    if save_and_plot:
        plot_scatter(f"coo_add_{tag}_{start}-{end}", nnz_totals,
                     "Total nnz (nnzA + nnzB)", nacho_runtimes,
                     pytorch=pytorch_runtimes)

    summarize(f"{tag} coo_add {start}-{end}", nacho_runtimes,
...
def main():
    ...
    for device in args.devices:
        benchmark_coo_add(args.start, args.end, device=device,
