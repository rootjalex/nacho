...
bool BaseKernelLowerer::exists_field_in_result_to_operand_pos_map(const Forall* forall, TensorLowerer& op_tensor) {
    ...
    if (result_tensor.tensor_level_exists(index) && result_tensor.is_sparse(index) && result_tensor.get_loop_num_for_next_sparse_level(BEFORE_FIRST_LOOP) == result_tensor.get_loop_num(index)) {
