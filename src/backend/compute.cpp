...
llir::lStmt ComputeKernelLowerer::lower_loop(
    ...
    auto make_loop = [&](const Seq &s) {
        ...
        auto get_body_epilogue_stmt = [&](const LoopNum loop_num, const CIN bodyCin) {
            ...
            bool has_next_forall = bodyCin.defined() && bodyCin.as<Forall>() != nullptr;
            ...
            if(has_next_forall && next_level != result_tensor.end_tensor_level() && result_tensor.is_sparse(next_level) && result_tensor.is_compressed(next_level)) {
            ...
            if(stmts.size()>0) {
                return (has_next_forall && next_level != result_tensor.end_tensor_level()) ? llir::IfElse::make(offset_write_cond, llir::Sequence::make(stmts), nullptr) : llir::Sequence::make(stmts);
            } else {
                return llir::lStmt();
            }
        };
        ...
        auto make_body = [&](const Seq &a) {
            ...
            auto epilogue_stmt = get_body_epilogue_stmt(loop_num, cin);
