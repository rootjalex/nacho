...
namespace backend {
    ...
    void CINLowerer::lower_cin() {
        ...
        for(int i=0; i< (int)sparse_intersection_levels.size()-1;i++) {
            ...
            for(int level = previous_sparse_intersection.get() + 1; level <= current_sparse_intersection.get(); level++) {
                for (auto it : operand_tensors) {
                    printer.print(it.second.lower_work_function(current_loop_order, LoopNum(level)));
                }
            }

            auto included_tensors = get_included_tensors_for_level(current_sparse_intersection.get());
            ...
            if(i!=(int)sparse_intersection_levels.size()-2){
                ...
                included_tensors = get_included_tensors_for_level(next_sparse_intersection.get());
            }
            ...
            bool has_precompute = result_tensor.tensor_type.format.get_prev_sparse_level(TensorLevelNum(current_sparse_intersection.get()+1)) != BEFORE_FIRST_LEVEL;
            if(has_precompute) {
        ...
        for (int phase = 0; phase < num_phases; phase++) {
            ...
            if (!is_last_phase && !phase_outermost_nnz.empty()) {
                ...
                BaseKernelLowerer base_lowerer(operand_tensors, result_tensor, empty_map,
                                               forall_list, BEFORE_FIRST_LOOP, BEFORE_FIRST_LOOP, BEFORE_FIRST_LOOP);
