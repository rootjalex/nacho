...
// Helper: allocate minimal buffers for empty NachoCOO results
static void alloc_empty_nacho_coo(int ndims, int **indices_ptrs, float *&values) {
    for (int d = 0; d < ndims; d++) {
        CHECK_CUDA(cudaMalloc((void **)&indices_ptrs[d], sizeof(int)));
    }
    CHECK_CUDA(cudaMalloc((void **)&values, sizeof(float)));
}
...
NachoCOO2D<int, float> nacho_coo2_add_nb(NachoCOO2D<int, float> A,
    ...
    coo2_add<int, float>(
        A.dim_i_size, A.dim_j_size,
        (int *)A.dim_i_indices.data(), (int *)A.dim_j_indices.data(),
        (float *)A.data.data(), (int)A.data.shape(0),
        B.dim_i_size, B.dim_j_size,
        (int *)B.dim_i_indices.data(), (int *)B.dim_j_indices.data(),
        (float *)B.data.data(), (int)B.data.shape(0),
        A.dim_i_size, A.dim_j_size,
        out_nnz, out_dim_j_indices, out_dim_i_indices, out_values);

    if (out_nnz == 0) {
        int *ptrs[2] = { out_dim_i_indices, out_dim_j_indices };
        alloc_empty_nacho_coo(2, ptrs, out_values);
        out_dim_i_indices = ptrs[0];
        out_dim_j_indices = ptrs[1];
    }

    return NachoCOO2D<int, float>(out_dim_i_indices, out_dim_j_indices,
                                   out_values, A.dim_i_size, A.dim_j_size,
                                   out_nnz);
}
...
NachoCOO3D<int, float> nacho_coo3_add_nb(NachoCOO3D<int, float> A,
    ...
    coo3_add<int, float>(
        A.dim_i_size, A.dim_j_size, A.dim_k_size,
        (int *)A.dim_i_indices.data(), (int *)A.dim_j_indices.data(),
        (int *)A.dim_k_indices.data(), (float *)A.data.data(),
        (int)A.data.shape(0),
        B.dim_i_size, B.dim_j_size, B.dim_k_size,
        (int *)B.dim_i_indices.data(), (int *)B.dim_j_indices.data(),
        (int *)B.dim_k_indices.data(), (float *)B.data.data(),
        (int)B.data.shape(0),
        A.dim_i_size, A.dim_j_size, A.dim_k_size,
        out_nnz, out_dim_k_indices, out_dim_j_indices,
        out_dim_i_indices, out_values);

    if (out_nnz == 0) {
        int *ptrs[3] = { out_dim_i_indices, out_dim_j_indices, out_dim_k_indices };
        alloc_empty_nacho_coo(3, ptrs, out_values);
        out_dim_i_indices = ptrs[0];
        out_dim_j_indices = ptrs[1];
        out_dim_k_indices = ptrs[2];
    }

    return NachoCOO3D<int, float>(out_dim_i_indices, out_dim_j_indices,
                                   out_dim_k_indices, out_values,
                                   A.dim_i_size, A.dim_j_size, A.dim_k_size,
                                   out_nnz);
}
...
NachoCOO4D<int, float> nacho_coo4_add_nb(NachoCOO4D<int, float> A,
    ...
    int *out_dim_j_indices;
    int *out_dim_k_indices;
    int *out_dim_l_indices;
    float *out_values;

    coo4_add<int, float>(
        A.dim_i_size, A.dim_j_size, A.dim_k_size, A.dim_l_size,
        (int *)A.dim_i_indices.data(), (int *)A.dim_j_indices.data(),
        (int *)A.dim_k_indices.data(), (int *)A.dim_l_indices.data(),
        (float *)A.data.data(), (int)A.data.shape(0),
        B.dim_i_size, B.dim_j_size, B.dim_k_size, B.dim_l_size,
        (int *)B.dim_i_indices.data(), (int *)B.dim_j_indices.data(),
        (int *)B.dim_k_indices.data(), (int *)B.dim_l_indices.data(),
        (float *)B.data.data(), (int)B.data.shape(0),
        A.dim_i_size, A.dim_j_size, A.dim_k_size, A.dim_l_size,
        out_nnz, out_dim_l_indices, out_dim_k_indices,
        out_dim_j_indices, out_dim_i_indices, out_values);

    if (out_nnz == 0) {
        int *ptrs[4] = { out_dim_i_indices, out_dim_j_indices,
                         out_dim_k_indices, out_dim_l_indices };
        alloc_empty_nacho_coo(4, ptrs, out_values);
        out_dim_i_indices = ptrs[0];
        out_dim_j_indices = ptrs[1];
        out_dim_k_indices = ptrs[2];
        out_dim_l_indices = ptrs[3];
    }

    return NachoCOO4D<int, float>(out_dim_i_indices, out_dim_j_indices,
                                   out_dim_k_indices, out_dim_l_indices,
                                   out_values, A.dim_i_size, A.dim_j_size,
                                   A.dim_k_size, A.dim_l_size, out_nnz);
}
